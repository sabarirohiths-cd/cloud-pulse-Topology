    ...
    def trace(self, vpc_id, sg_ids, root_id, root_tags=None, root_name=None):
        ...
        try:
            ...
            for page in paginator.paginate():
                for rds in page.get('DBInstances', []):
                    ...
                    if rds.get('DBSubnetGroup', {}).get('VpcId') == vpc_id:
                        ...
                        try:
                            ...
                            if linked:
                                ...
                                db_port = rds.get('Endpoint', {}).get('Port', 'unknown')
                                self.add_edge(root_id, rds_id, 'QUERIES', health_state="BLOCKED", diagnostic=f"Missing SG Ingress Rule on Port {db_port}")
            ...
            if e.response['Error']['Code'] in ['AccessDenied', 'UnauthorizedOperation']:
                if hasattr(self.fetcher, 'warnings'):
                    self.fetcher.warnings.append("Warning: Could not trace Databases due to missing IAM permissions (rds:DescribeDBInstances).")
